...
def update_mp3_metadata(file_path):
    ...
    If the existing metadata matches the extracted values and the force switch is off,
    the file is skipped. Otherwise, the metadata is updated and verified.
    ...
    if not force_rewrite and current_artist == artist and current_album == album:
        print("Existing metadata matches extracted values. Skipping update.")
        return ("skipped", "Metadata matches extracted values")
    else:
        if force_rewrite:
            print("Force rewrite enabled. Rewriting metadata even though it matches.")
        elif current_artist or current_album:
...
def main():
    global extraction_method_func, force_rewrite
    ...
    # Menu: ask for force rewrite option
    print("\nDo you want to force rewriting metadata even if they already match the extracted values?")
    print("1. Yes (force rewrite)")
    print("2. No (skip if matching)")
    force_choice = input("Enter your choice [1/2]: ").strip()
    if force_choice == "1":
        force_rewrite = True
    else:
        force_rewrite = False
