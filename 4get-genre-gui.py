...
def cover_exists(file_path):
    ...
    if file_path.lower().endswith('.mp3'):
        try:
            ...
            audio = ID3(file_path)
            if any(frame for frame in audio.values() if frame.FrameID == "APIC"):
                return True
        except Exception:
            return False
...
class WorkerThread(QThread):
    ...
    def run(self):
        ...
        for file_path in files:
            ...
            # Check if genre metadata already exists.
            audio = MutagenFile(file_path, easy=True)
            if audio is not None and "genre" in audio and audio["genre"]:
                ...
                # If cover image is missing, attempt to retrieve cover image via API.
                if not cover_exists(file_path):
                    self.log_signal.emit("No cover image found, attempting to retrieve cover image...")
                    try:
                ...
                # No genre metadata exists; call API to get both genre and cover.
                try:
                    info = get_book_info_google(title, self.api_key)
                    self.api_count += 1
                    self.api_count_signal.emit(self.api_count)
                ...
                if info:
                    ...
                    categories = None
                    cover_url = None
                if cover_url and not cover_exists(file_path):
                    if write_cover_to_file(file_path, cover_url):
                        self.log_signal.emit("Cover image updated successfully.")
                    else:
                        self.log_signal.emit("Failed to update cover image.")

            # If we have genre information, move the file.
            if categories:
                # Move file to a subfolder (named for the genre) at the parent folder level.
                parent_folder = os.path.dirname(self.folder)
                genre_folder = os.path.join(parent_folder, categories[0])
