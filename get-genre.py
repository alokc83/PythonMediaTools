...
def get_book_categories_google(book_title, api_key="AIzaSyDXOoTjLadczBJ0Da2Ny1DD9VVbilAgrxg", delay=1):
    """
    Query the Google Books API for a given book title and return the categories
    from the first result that has them. Uses the provided API key ("abc1234")
    and adds a delay between requests to help avoid rate limiting.
    """
...
def main():
    print("=== Book Genre Retriever ===")
    user_input = input("Enter the book title (or press Enter to use a directory of audiobooks): ").strip()
    
    # The Google Books API key is now set to "abc1234" in the function calls.
    if user_input:
        ...
        for file_path in tqdm(audio_files, desc="Processing files", unit="file"):
            ...
            if categories:
                print("Categories (via Google Books):")
                for cat in categories:
                    print(f" - {cat}")
            else:
                ...
                if categories:
                    print("Subjects (via OpenLibrary):")
                    for subj in categories:
                        print(f" - {subj}")
                else:
