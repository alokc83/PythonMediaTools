...
def get_genre_from_file(item):
    """
    Use mutagen to read the genre metadata from the actual mp3/m4a file.
    Returns a list of genres if available.
    """
    for media in item.media:
        for part in media.parts:
            if part.file.lower().endswith(('.mp3', '.m4a')):
...
def main():
    ...
    # Convert library search results to a list so we can measure progress.
    ...
    for item in tqdm(all_items, desc="Processing audio items", unit="item"):
        # Only process items that are audio files (MP3/M4A).
        if not get_audio_file(item):
            continue

        # Attempt to retrieve genre metadata from Plex data.
        genres = None
        ...
        # If Plex metadata doesn't include genre info, try reading directly from the file.
        if not genres or (isinstance(genres, list) and not genres):
            file_genres = get_genre_from_file(item)
            if file_genres:
                genres = file_genres

        # If still no genre, optionally use Open Library as a fallback.
        if not genres or (isinstance(genres, list) and not genres):
        ...
        for g in genres:
            # 'g' may be an object with a 'tag' property or a simple string.
