...
def extract_artist_album(file_path):
    ...
    Priority:
    - If the filename (without extension) ends with a parenthesized group, that group is taken as the artist,
      and the preceding text is taken as the album.
    - Otherwise, split the filename at the first '-' character:
    ...
    # First, try to match the pattern: "album (artist)" at the end of the filename
    pattern = r'^(.*?)\s*\(([^)]+)\)\s*$'
    ...
    # Fallback: split by the first hyphen
...
def update_mp3_metadata(file_path):
    ...
    Checks if both artist and album tags already exist and match the extracted values.
    If they match, the file is skipped.
    Otherwise, the metadata is updated.
    
    Returns a tuple (status, reason) where status is one of:
      - "updated"  : tags updated and verified successfully.
      - "skipped"  : file was skipped (either due to filename format or because metadata already matches).
    ...
    if artist is None or album is None:
        msg = "No valid pattern found (expected 'album (artist)' or 'artist - album')"
    ...
    # Check if metadata already exists and matches the extracted values
    ...
    if current_artist == artist and current_album == album:
        print("Metadata already exists and matches the extracted values. Skipping rewriting.")
        return ("skipped", "Metadata already exists and matches")
    else:
        if current_artist or current_album:
            print("Metadata exists but does not match extracted values. Updating metadata.")
