...
def analyze_folder(folder_path, title_dict):
    ...
    Analyzes the folder's title dictionary to determine:
      - Number of unique titles (distinct keys).
    ...
    print(f"\nFolder: {folder_path}")
    print(f"  Unique Titles (distinct): {unique_titles}")
...
def list_unique_titles_by_folder(folder_data):
    """
    For each folder in the provided folder_data (which maps folder path to a
    title dictionary), lists the titles that occur exactly once (unique within
    that folder).
    """
    print("\nUnique Titles in each folder (appear only once):")
    for folder, title_dict in folder_data.items():
        unique_list = [title for title, files in title_dict.items() if len(files) == 1]
        print(f"\nFolder: {folder}")
        if unique_list:
...
def main():
    ...
    # New: List the unique titles (titles that occur only once) from each folder.
    list_unique_titles_by_folder(folder_data)
