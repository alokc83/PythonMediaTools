...
# Delete empty directories upward until ROOT (never delete ROOT)
cleanup_empty_upwards() {
  local start_dir="$1"
  ...
  while :; do
    ...
    # If directory has anything (including hidden), stop
    if ls -A "$d" >/dev/null 2>&1 && [[ -n "$(ls -A "$d" 2>/dev/null)" ]]; then
      break
    fi

    if [[ "$DRYRUN" == "1" ]]; then
      printf '%s\n' "Would delete empty dir: $d" >> "$deleted_dirs_report"
      # In dryrun, do not actually remove, and stop to avoid pretending parents are empty
      break
    else
      rmdir "$d" 2>/dev/null || break
      printf '%s\n' "Deleted empty dir: $d" >> "$deleted_dirs_report"
    fi
...
if [[ "$DRYRUN" == "1" ]]; then
  ...
  echo "Mode: move + rename + cleanup empty dirs" >&2
fi
echo "" >&2
...
if [[ "$total_files" -gt 0 ]]; then
  while IFS= read -r -d '' src; do
    ...
    # Cleanup empty source dirs after moving
    cleanup_empty_upwards "$src_dir"
  done < "$files_list"
...
# Count deleted dirs from report (only true deletes, not dryrun lines)
if [[ -f "$deleted_dirs_report" ]]; then
  deleted_dirs_count="$(grep -c '^Deleted empty dir:' "$deleted_dirs_report" 2>/dev/null || true)"
else
  deleted_dirs_count=0
fi
...
echo "Empty source dirs deleted: $deleted_dirs_count" >&2
