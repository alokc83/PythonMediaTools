...
def list_audio_files_recursive(root: str, exts: set) -> List[str]:
  ...
  for dirpath, _dirnames, filenames in os.walk(root):
    seen_dirs += 1
    progress_line("ScanTree", seen_dirs, max(1, total_dirs), dirpath)
    for name in filenames:
      ext = os.path.splitext(name)[1].lstrip(".").lower()
      if ext in exts:
        out.append(os.path.join(dirpath, name))

  finish_phase(f"ScanTree done. Audio files found: {len(out)}")
  return out
...
def task_duplicates_and_move(dirs: List[str], exts: set, out_dir: str, show_groups: int, dry_run: bool) -> None:
  ...
  for idx, (_k, files) in enumerate(items, 1):
    progress_line("PickKeep", idx, len(items), files[0].title_display)
    keep_plan.append((_k, choose_keep(files), files))
...
def task_rename_to_title(dirs: List[str], exts: set, dry_run: bool) -> None:
  ...
  print(f"Skipped (already matching title): {len(paths) - len(plan) - missing_title}")
...
def task_flatten_to_root(root_dir: str, exts: set, dry_run: bool, cleanup_dirs: bool) -> None:
  ...
  # Phase 1: rename audio already in root to Title using ONLY title tag
  root_files: List[str] = []
  try:
