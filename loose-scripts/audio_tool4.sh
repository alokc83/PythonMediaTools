...
def task_file_to_dir(target_dir: str, exts: set, dry_run: bool) -> None:
  # Matches file-to-dir.sh behavior:
  # current level only, create folder named after file base, do not overwrite
  ...
  for idx, src in enumerate(files, 1):
    progress_line("FileToDir", idx, len(files), src)
...
def main() -> int:
  ...
  if choice == "4":
    ...
    if not target_dir:
      print("Directory is required for option 4.")
      return 2
    ...
    task_file_to_dir(target_dir, exts, dry_run)
    return 0
