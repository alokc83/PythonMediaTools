...
# We process MP3 files only (they will later be converted to M4A)
ALLOWED_EXT = {".mp3"}
...
def print_script_info():
    ...
    This script processes MP3 files in a specified folder as follows:
      ...
      1. Metadata Update:
         - For each MP3 file, its current ID3v2 metadata is displayed for review.
         - You are then asked to confirm updating that file.
         - If the "title" (TIT2) is missing but "album" (TALB) exists (or vice versa),
           the missing tag is filled using the available one. If both are missing, the
           file name (without extension) is used for both.
         ...
         - **Cover Art:**  
             * If no album cover (APIC) exists OR if an existing cover is lower than 800×800 pixels,
               the script will attempt to fetch a high-quality cover.
             * It first tries Blinkist.com by constructing a URL from the title (slugified and
               suffixed with "-en"). Debug messages are shown.
             * If Blinkist’s image is not high quality, it falls back to Audible, then Goodreads,
               and finally Google Images as a last resort.
             * If cover fetching fails, you will be prompted whether to continue processing.
...
def display_id3_metadata(file_path):
    try:
        tags = ID3(file_path)
        print(f"\nCurrent metadata for '{file_path}':")
        for key, frame in tags.items():
            print(f"  {key}: {frame}")
        print("-" * 40)
    except Exception as e:
        print(f"\nCould not read metadata for '{file_path}': {e}")
...
def process_metadata(operation_folder):
    ...
    for file_path in tqdm(mp3_files, desc="Updating Metadata", unit="file"):
        # Display current metadata for review
        display_id3_metadata(file_path)
...
def fetch_album_cover(title):
    ...
    tqdm.write("[DEBUG] Falling back to Google Images as last resort for cover art...")
    try:
        search_url = f"https://www.google.com/search?tbm=isch&q={urllib.parse.quote(title)}"
        r = requests.get(search_url, headers=headers, timeout=10)
        tqdm.write(f"[DEBUG] Google Images HTTP status: {r.status_code}")
        if r.status_code == 200:
            from bs4 import BeautifulSoup
            soup = BeautifulSoup(r.text, "html.parser")
            ...
            for img in imgs:
                ...
                if src and src.startswith("http"):
                    ...
                    if r2.status_code == 200 and check_image_quality(r2.content):
                        tqdm.write(f"[DEBUG] Google Images cover found: {src}")
                        return r2.content
    except Exception as e:
        tqdm.write(f"[DEBUG] Google Images search failed: {e}")
