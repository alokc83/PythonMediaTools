...
def print_script_info():
    ...
    This script processes MP3 files in a specified folder as follows:
      ...
      1. Metadata Update:
         - For each MP3 file, the script first displays its current ID3v2 metadata.
         - You are then asked to confirm whether to update the metadata for that file.
         ...
         - If no album cover (APIC) exists, the script attempts to fetch one:
             * It first tries Blinkist.com by constructing a URL from the title 
               (slugifying it and appending "-en"). Debug messages are printed.
             * If Blinkist fails, it falls back to Audible, then Goodreads.
             * If cover fetching fails, you will be prompted whether to continue.
...
def display_id3_metadata(file_path):
    """Reads and prints the current ID3v2 metadata for the given file."""
    try:
        tags = ID3(file_path)
            ...
            if not any(key.startswith("APIC") for key in tags.keys()):
                ...
                if answer_fetch == "y":
                    ...
                    if cover:
                        ...
                        answer_continue = input(f"Failed to fetch cover art for '{file_path}'. Continue processing remaining files? (y/n): ").strip().lower()
...
def main():
    ...
    # Phase 1: Update metadata for all MP3 files.
